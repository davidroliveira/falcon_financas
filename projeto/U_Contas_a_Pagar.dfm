inherited FrameContas_a_Pagar: TFrameContas_a_Pagar
  Width = 1172
  Height = 710
  OnDestroy = UniFrameDestroy
  ExplicitWidth = 1172
  ExplicitHeight = 710
  inherited UniToolBar1: TUniToolBar
    Width = 1172
    ExplicitWidth = 1172
    object bt_pagar: TUniToolButton
      Left = 312
      Top = 0
      Hint = 'Realizar Pagamento'
      Visible = False
      ScreenMask.Enabled = True
      ScreenMask.WaitData = True
      ScreenMask.Message = 'Processando...'
      ShowHint = True
      ParentShowHint = False
      ImageIndex = 10
      Caption = 'bt_pagar'
      OnClick = bt_pagarClick
    end
    object bt_anexar: TUniToolButton
      Left = 351
      Top = 0
      Hint = 'Anexar arquivo'
      ScreenMask.Enabled = True
      ScreenMask.WaitData = True
      ScreenMask.Message = 'Processando...'
      ShowHint = True
      ParentShowHint = False
      ImageIndex = 9
      Caption = 'bt_anexar'
      OnClick = bt_anexarClick
    end
  end
  inherited UniPageControl1: TUniPageControl
    Width = 1172
    Height = 651
    ExplicitTop = 44
    ExplicitWidth = 1172
    ExplicitHeight = 511
    inherited Tab_Cadastro: TUniTabSheet
      ExplicitLeft = 4
      ExplicitTop = 24
      ExplicitWidth = 1164
      ExplicitHeight = 623
      inherited ScrollBox: TUniScrollBox
        Width = 1164
        Height = 623
        ExplicitLeft = 1
        ExplicitTop = -2
        ExplicitWidth = 1164
        ExplicitHeight = 483
        object pn_tipo: TUniPanel
          Left = 0
          Top = 0
          Width = 1164
          Height = 35
          Hint = ''
          Caption = ''
          Align = alTop
          Anchors = [akLeft, akTop, akRight]
          Color = clWhite
          TabOrder = 1
          object rdb_tip_normal: TUniRadioButton
            Left = 7
            Top = 4
            Width = 73
            Height = 17
            Hint = 'Contas Normais Vencem apenas uma '#250'nica vez.'
            ShowHint = True
            ParentShowHint = False
            Checked = True
            Caption = 'Normal'
            TabOrder = 1
            ParentFont = False
            Font.Color = clHotLight
            Font.Height = -13
            Font.Name = 'Segoe UI'
            Font.Style = [fsBold]
            OnClick = rdb_tip_normalClick
          end
          object rdb_tip_recorrente: TUniRadioButton
            Left = 112
            Top = 4
            Width = 91
            Height = 17
            Hint = 
              'Contas Recorrentes sempre ir'#227'o existir, ex: conta de telefone, l' +
              'uz, mensalidade academia.'
            ShowHint = True
            ParentShowHint = False
            Checked = False
            Caption = 'Recorrente'
            TabOrder = 2
            ParentFont = False
            Font.Color = clHotLight
            Font.Height = -13
            Font.Name = 'Segoe UI'
            Font.Style = [fsBold]
            OnClick = rdb_tip_normalClick
          end
          object rdb_tip_parcelado: TUniRadioButton
            Left = 241
            Top = 4
            Width = 86
            Height = 17
            Hint = 'Contas Parceladas possuem um n'#250'mero fixo de vencimentos.'
            ShowHint = True
            ParentShowHint = False
            Checked = False
            Caption = 'Parcelada'
            TabOrder = 3
            ParentFont = False
            Font.Color = clHotLight
            Font.Height = -13
            Font.Name = 'Segoe UI'
            Font.Style = [fsBold]
            OnClick = rdb_tip_normalClick
          end
          object UniImage2: TUniImage
            Left = 73
            Top = 8
            Width = 16
            Height = 16
            Hint = 'Contas Normais Vencem apenas uma '#250'nica vez.'
            ShowHint = True
            ParentShowHint = False
            Center = True
            Stretch = False
            Proportional = False
            Url = '/imagens/16/status_grey_N.png'
            Transparent = False
            OnClick = UniImage1Click
          end
          object UniImage3: TUniImage
            Left = 199
            Top = 8
            Width = 16
            Height = 16
            Hint = 
              'Contas Recorrentes sempre ir'#227'o existir, ex: conta de telefone, l' +
              'uz, mensalidade academia.'
            ShowHint = True
            ParentShowHint = False
            Center = True
            Stretch = False
            Proportional = False
            Url = '/imagens/16/status_grey_R.png'
            Transparent = False
            OnClick = UniImage1Click
          end
          object UniImage4: TUniImage
            Left = 321
            Top = 8
            Width = 16
            Height = 16
            Hint = 'Contas Parceladas possuem um n'#250'mero fixo de vencimentos.'
            ShowHint = True
            ParentShowHint = False
            Center = True
            Stretch = False
            Proportional = False
            Url = '/imagens/16/status_grey_P.png'
            Transparent = False
            OnClick = UniImage1Click
          end
        end
        object pn_principal: TUniPanel
          Left = 0
          Top = 35
          Width = 1164
          Height = 137
          Hint = ''
          Caption = ''
          Align = alTop
          Anchors = [akLeft, akTop, akRight]
          Color = clBtnFace
          TabOrder = 2
          DesignSize = (
            1164
            137)
          object UniLabel1: TUniLabel
            Left = 7
            Top = 12
            Width = 54
            Height = 15
            Hint = ''
            Caption = 'Descri'#231#227'o:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 9
          end
          object UniLabel2: TUniLabel
            Left = 448
            Top = 12
            Width = 83
            Height = 15
            Hint = ''
            Caption = 'N'#186' Documento:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 10
          end
          object cmb_tipo_despesa: TComboBoxYeni
            Tag = 1
            Left = 108
            Top = 71
            Width = 324
            Height = 23
            Hint = ''
            Style = csDropDown
            ClientEvents.ExtEvents.Strings = (
              
                'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                'er.allowBlank=false;'#13#10'  event: sender.emptyText='#39'Informe o Tipo ' +
                'de Despesa'#39';'#13#10'}')
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            MaxLength = 0
            ParentFont = False
            RemoteQuery = False
            TabOrder = 6
            OnClick = cmb_tipo_despesaClick
          end
          object UniLabel3: TUniLabel
            Left = 7
            Top = 74
            Width = 89
            Height = 15
            Hint = ''
            Caption = 'Tipo de Despesa:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 11
          end
          object UniLabel4: TUniLabel
            Left = 7
            Top = 103
            Width = 63
            Height = 15
            Hint = ''
            Caption = 'Fornecedor:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 12
          end
          object cmb_fornecedor: TComboBoxYeni
            Tag = 1
            Left = 108
            Top = 100
            Width = 324
            Height = 23
            Hint = ''
            Style = csDropDown
            ClientEvents.ExtEvents.Strings = (
              
                'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                'er.allowBlank=false;'#13#10'  event: sender.emptyText='#39'Informe o Forne' +
                'cedor'#39';'#13#10'}')
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            MaxLength = 0
            ParentFont = False
            RemoteQuery = False
            TabOrder = 7
            OnClick = cmb_fornecedorClick
          end
          object UniLabel5: TUniLabel
            Left = 260
            Top = 42
            Width = 67
            Height = 15
            Hint = ''
            Caption = 'Vencimento:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 13
          end
          object lb_n_parcelas: TUniLabel
            Left = 653
            Top = 42
            Width = 63
            Height = 15
            Hint = ''
            Visible = False
            Caption = 'N'#186' Parcelas:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 14
          end
          object UniLabel7: TUniLabel
            Left = 7
            Top = 42
            Width = 30
            Height = 15
            Hint = ''
            Caption = 'Valor:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 15
          end
          object lb_forma_pagamento: TUniLabel
            Left = 448
            Top = 42
            Width = 44
            Height = 15
            Hint = ''
            Visible = False
            Caption = 'Periodo:'
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 16
          end
          object DB_Descricao: TUniDBEdit
            Tag = 1
            Left = 108
            Top = 10
            Width = 324
            Height = 22
            Hint = ''
            DataField = 'DESCRICAO_DOCUM'
            DataSource = DS
            PasswordChar = #0
            CharCase = ecNormal
            MaxLength = 0
            CharEOL = #0
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 0
            Color = clWindow
            ClientEvents.ExtEvents.Strings = (
              
                'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                'er.allowBlank=false;'#13#10'  event: sender.emptyText='#39'Defina um nome ' +
                'para sua despesa...'#39';'#13#10'}')
          end
          object DB_Data_Vencimento: TUniDBDateTimePicker
            Left = 331
            Top = 39
            Width = 101
            Height = 22
            Hint = ''
            DateTime = 41483.000000000000000000
            DateFormat = 'dd/MM/yyyy'
            TimeFormat = 'HH:mm:ss'
            Kind = tUniDate
            FirstDayOfWeek = dowLocaleDefault
            TabOrder = 3
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            ParentColor = False
            Color = clWhite
            ClientEvents.ExtEvents.Strings = (
              
                'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                'er.allowBlank=false;'#13#10'}')
            DataField = 'DAT_VENCIMENTO'
            DataSource = DS
          end
          object DB_Periodo: TUniDBComboBox
            Left = 535
            Top = 38
            Width = 104
            Height = 23
            Hint = ''
            Visible = False
            DataField = 'PERIODO'
            DataSource = DS
            Style = csDropDown
            MaxLength = 0
            Items.Strings = (
              'Semanal'
              'Quinzenal'
              'Mensal'
              'Trimestral'
              'Semestral'
              'Anual')
            ItemIndex = -1
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 4
          end
          object DB_Documento: TUniDBEdit
            Left = 536
            Top = 8
            Width = 103
            Height = 22
            Hint = ''
            DataField = 'NUM_DOCUM'
            DataSource = DS
            PasswordChar = #0
            CharCase = ecNormal
            MaxLength = 0
            CharEOL = #0
            ParentFont = False
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Segoe UI'
            TabOrder = 1
            Color = clWindow
          end
          object UniSpeedButton1: TUniSpeedButton
            Left = 438
            Top = 69
            Width = 25
            Height = 25
            Hint = 'Realiza o Cadastro de Tipo de Despesa.'
            ScreenMask.Enabled = True
            ScreenMask.WaitData = True
            ScreenMask.Message = 'Carregando Tela Solicitada, Aguarde...'
            ShowHint = True
            ParentShowHint = False
            AllowAllUp = False
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Caption = ''
            ParentColor = False
            Color = clBtnFace
            OnClick = UniSpeedButton1Click
          end
          object UniSpeedButton2: TUniSpeedButton
            Left = 438
            Top = 98
            Width = 25
            Height = 25
            Hint = 'Realiza o Cadastro de Fornecedor.'
            ScreenMask.Enabled = True
            ScreenMask.WaitData = True
            ScreenMask.Message = 'Carregando Tela Solicitada, Aguarde...'
            ShowHint = True
            ParentShowHint = False
            AllowAllUp = False
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Caption = ''
            ParentColor = False
            Color = clBtnFace
            OnClick = UniSpeedButton2Click
          end
          object DB_Parcelas: TUniDBNumberEdit
            Left = 719
            Top = 39
            Width = 49
            Height = 22
            Hint = ''
            Visible = False
            DataField = 'NUM_PARCELAS'
            DataSource = DS
            MaxLength = 0
            CharEOL = #0
            TabOrder = 5
            Color = clWindow
            ClientEvents.ExtEvents.Strings = (
              
                'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                'er.allowBlank=false;'#13#10'  event: sender.MaxValue=300; '#13#10'}')
            OnExit = DB_ParcelasExit
          end
          object DB_Valor: TUniDBEdit
            Tag = 1
            Left = 108
            Top = 38
            Width = 121
            Height = 22
            Hint = ''
            DataField = 'VAL_PARCELA'
            DataSource = DS
            PasswordChar = #0
            CharCase = ecNormal
            MaxLength = 0
            CharEOL = #0
            ParentFont = False
            Font.Color = clRed
            Font.Style = [fsBold]
            TabOrder = 2
            Color = clWindow
            ClientEvents.ExtEvents.Strings = (
              
                'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                'er.allowBlank=false; '#13#10'  event: sender.emptyText='#39'R$ 0,00'#39';'#13#10'}')
            OnExit = DB_ValorExit
          end
          object lb_parcela: TUniLabel
            Left = 720
            Top = 64
            Width = 62
            Height = 14
            Hint = ''
            Alignment = taCenter
            Caption = 'lb_parcela'
            ParentFont = False
            Font.Color = clHotLight
            Font.Height = -12
            Font.Style = [fsBold]
            TabOrder = 19
          end
          object pn_inf_anexo: TUniPanel
            Left = 823
            Top = 1
            Width = 335
            Height = 135
            Hint = ''
            BorderStyle = ubsNone
            TitleVisible = True
            Title = 'Anexos'
            Caption = ''
            Collapsible = True
            Anchors = [akTop, akRight]
            Color = clBtnFace
            TabOrder = 20
            object UniDBGrid2: TUniDBGrid
              Left = 0
              Top = 34
              Width = 335
              Height = 101
              Cursor = crHandPoint
              Hint = 'Duplo Clique para baixar o anexo.'
              ScreenMask.Enabled = True
              ScreenMask.WaitData = True
              ScreenMask.Message = 'Carregando Anexos, Aguarde...'
              ScreenMask.Target = UniDBGrid2
              ShowHint = True
              ParentShowHint = False
              DataSource = DSAnexo
              Options = []
              ReadOnly = True
              WebOptions.Paged = False
              WebOptions.LoadMask = True
              WebOptions.LoadMaskMsg = 'Carregando, Por Favor Aguarde...'
              HeaderTitleAlign = taLeftJustify
              OnDblClick = UniDBGrid2DblClick
              StripeRows = False
              Align = alBottom
              Anchors = [akLeft, akRight, akBottom]
              Font.Color = clHotLight
              ParentFont = False
              TabOrder = 1
              Columns = <
                item
                  FieldName = 'NOME_ANEXO'
                  Title.Caption = 'Anexo'
                  Width = 300
                  Visible = True
                  Font.Color = clHotLight
                  Expanded = False
                end>
            end
            object bt_anexar_mais: TUniBitBtn
              Left = 3
              Top = 3
              Width = 25
              Height = 25
              Hint = 'Anexar arquivo'
              ShowHint = True
              ParentShowHint = False
              Glyph.Data = {
                
                110D0A0A0A040101010101010101010110090909100101010101}
              Caption = ''
              TabOrder = 2
              OnClick = bt_anexar_maisClick
            end
            object bt_excluir_anexo: TUniBitBtn
              Left = 37
              Top = 3
              Width = 25
              Height = 25
              Hint = 'Excluir Anexo'
              ShowHint = True
              ParentShowHint = False
              Glyph.Data = {
                
                37BF2737BF2F3EC2838CDBFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              Caption = ''
              TabOrder = 3
              OnClick = bt_excluir_anexoClick
            end
          end
        end
        object pn_inf_pagamento: TUniPanel
          Left = 0
          Top = 172
          Width = 1164
          Height = 137
          Hint = ''
          Visible = False
          TitleVisible = True
          Title = 'Informa'#231#245'es de Pagamento'
          Caption = ''
          Collapsible = True
          Align = alTop
          Anchors = [akLeft, akTop, akRight]
          Color = clBtnFace
          TabOrder = 3
          object UniDBGrid1: TUniDBGrid
            Left = 5
            Top = 3
            Width = 659
            Height = 90
            Hint = ''
            DataSource = DSPagamento
            Options = [dgTitles, dgColumnResize, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
            WebOptions.Paged = False
            WebOptions.LoadMask = True
            WebOptions.LoadMaskMsg = 'Carregando, Por Favor Aguarde...'
            HeaderTitleAlign = taLeftJustify
            StripeRows = False
            TabOrder = 1
            Columns = <
              item
                FieldName = 'DAT_PAGAMENTO'
                Title.Caption = 'Pagamento'
                Width = 70
                Visible = True
                Expanded = False
              end
              item
                FieldName = 'VALOR'
                Title.Caption = 'Valor Pago'
                Width = 100
                Visible = True
                Alignment = taRightJustify
                Expanded = False
              end
              item
                FieldName = 'TIPO'
                Title.Caption = 'Tipo'
                Width = 125
                Visible = True
                ReadOnly = True
                Expanded = False
              end
              item
                FieldName = 'NOME_CONTA'
                Title.Caption = 'Nome Conta'
                Width = 150
                Visible = True
                ReadOnly = True
                Expanded = False
              end
              item
                FieldName = 'NOME_BANCO'
                Title.Caption = 'Banco'
                Width = 185
                Visible = True
                ReadOnly = True
                Expanded = False
              end>
          end
          object bt_remove_baixa: TUniBitBtn
            Left = 670
            Top = 3
            Width = 25
            Height = 25
            Hint = 'Excluir Pagamento'
            ShowHint = True
            ParentShowHint = False
            Glyph.Data = {
              
              37BF2737BF2F3EC2838CDBFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Caption = ''
            TabOrder = 2
            OnClick = bt_remove_baixaClick
          end
          object bt_informacoes: TUniBitBtn
            Left = 670
            Top = 68
            Width = 25
            Height = 25
            Hint = 'Informar Descontos, Juros ou Multas.'
            ShowHint = True
            ParentShowHint = False
            Caption = '...'
            TabOrder = 3
            OnClick = bt_informacoesClick
          end
          object pn_inf_juros: TUniPanel
            Left = 714
            Top = 3
            Width = 197
            Height = 90
            Hint = ''
            Visible = False
            Caption = ''
            Color = clBtnFace
            TabOrder = 4
            object UniLabel13: TUniLabel
              Left = 11
              Top = 9
              Width = 72
              Height = 13
              Hint = ''
              Caption = 'Valor de Multa:'
              ParentFont = False
              Font.Color = clBlack
              TabOrder = 1
            end
            object UniLabel14: TUniLabel
              Left = 11
              Top = 38
              Width = 72
              Height = 13
              Hint = ''
              Caption = 'Valor de Juros:'
              ParentFont = False
              Font.Color = clBlack
              TabOrder = 2
            end
            object UniLabel15: TUniLabel
              Left = 11
              Top = 67
              Width = 91
              Height = 13
              Hint = ''
              Caption = 'Valor de Desconto:'
              ParentFont = False
              Font.Color = clBlack
              TabOrder = 3
            end
            object DB_Val_Multa: TUniDBEdit
              Left = 108
              Top = 6
              Width = 76
              Height = 22
              Hint = ''
              DataField = 'VAL_MULTA'
              DataSource = DS
              PasswordChar = #0
              CharCase = ecNormal
              MaxLength = 0
              CharEOL = #0
              TabOrder = 4
              Color = clWindow
              ClientEvents.ExtEvents.Strings = (
                
                  'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                  'er.allowBlank=false; '#13#10'  event: sender.emptyText='#39'R$ 0,00'#39';'#13#10'}')
            end
            object DB_Val_Juros: TUniDBEdit
              Left = 108
              Top = 34
              Width = 76
              Height = 22
              Hint = ''
              DataField = 'VAL_JUROS'
              DataSource = DS
              PasswordChar = #0
              CharCase = ecNormal
              MaxLength = 0
              CharEOL = #0
              TabOrder = 5
              Color = clWindow
              ClientEvents.ExtEvents.Strings = (
                
                  'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                  'er.allowBlank=false; '#13#10'  event: sender.emptyText='#39'R$ 0,00'#39';'#13#10'}')
            end
            object DB_Val_Desconto: TUniDBEdit
              Left = 108
              Top = 62
              Width = 76
              Height = 22
              Hint = ''
              DataField = 'VAL_DESCONTO'
              DataSource = DS
              PasswordChar = #0
              CharCase = ecNormal
              MaxLength = 0
              CharEOL = #0
              TabOrder = 6
              Color = clWindow
              ClientEvents.ExtEvents.Strings = (
                
                  'OnBeforerender=function OnBeforerender(sender)'#13#10'{'#13#10'  event: send' +
                  'er.allowBlank=false; '#13#10'  event: sender.emptyText='#39'R$ 0,00'#39';'#13#10'}')
            end
          end
          object UniDBText1: TUniDBText
            Left = 115
            Top = 92
            Width = 73
            Height = 13
            Hint = ''
            DataField = 'VAL_PAGO'
            DataSource = DS
            Alignment = taRightJustify
            ParentFont = False
            Font.Color = clGreen
            Font.Style = [fsBold]
          end
          object UniLabel16: TUniLabel
            Left = 7
            Top = 92
            Width = 63
            Height = 13
            Hint = ''
            Caption = 'Valor Pago:'
            ParentFont = False
            Font.Color = clGreen
            Font.Style = [fsBold]
            TabOrder = 6
          end
        end
      end
    end
    inherited Tab_Consulta: TUniTabSheet
      ExplicitLeft = 4
      ExplicitTop = 24
      ExplicitWidth = 1164
      ExplicitHeight = 623
      object dbg_pesquisa: TUniDBGrid
        Left = 0
        Top = 156
        Width = 1164
        Height = 467
        Hint = ''
        DataSource = DS
        Options = [dgTitles, dgColumnResize, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ReadOnly = True
        WebOptions.LoadMask = True
        WebOptions.LoadMaskMsg = 'Carregando, Por Favor Aguarde...'
        Summary.Enabled = True
        HeaderTitleAlign = taLeftJustify
        OnColumnSort = dbg_pesquisaColumnSort
        OnDblClick = dbg_pesquisaDblClick
        OnMouseDown = dbg_pesquisaMouseDown
        StripeRows = False
        Align = alClient
        Anchors = [akLeft, akTop, akRight, akBottom]
        TabOrder = 0
        Columns = <
          item
            FieldName = 'IMG_STATUS'
            Title.Alignment = taCenter
            Title.Caption = '<img src="../imagens/16/indicador_bottom.png" />'
            Width = 40
            Visible = True
            Alignment = taCenter
            Expanded = False
          end
          item
            FieldName = 'DESCRICAO_DOCUM'
            Title.Caption = 'Descri'#231#227'o'
            Width = 200
            Visible = True
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'DAT_VENCIMENTO'
            Title.Caption = 'Vencimento'
            Width = 70
            Visible = True
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'VAL_PARCELA'
            Title.Caption = 'Valor a Pagar'
            Width = 100
            Visible = True
            Alignment = taRightJustify
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'VAL_PAGO'
            Title.Caption = 'Valor Pago'
            Width = 100
            Visible = True
            Alignment = taRightJustify
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'DAT_PAGAMENTO'
            Title.Caption = 'Pagamento'
            Width = 70
            Visible = True
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'IMG_TIPO'
            Title.Alignment = taCenter
            Title.Caption = 'Tipo'
            Width = 60
            Visible = True
            Alignment = taCenter
            Expanded = False
          end
          item
            FieldName = 'NUM_DOCUM'
            Title.Caption = 'N'#186' Documento'
            Width = 75
            Visible = True
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'DESCRICAO_DESPESA'
            Title.Caption = 'Tipo de Despesa'
            Width = 175
            Visible = True
            Expanded = False
            Sortable = True
          end
          item
            FieldName = 'RAZAO_SOCIAL'
            Title.Caption = 'Fornecedor'
            Width = 175
            Visible = True
            Expanded = False
            Sortable = True
          end>
      end
      object pn_filtros_avancados: TUniPanel
        Left = 0
        Top = 42
        Width = 1164
        Height = 114
        Hint = ''
        Visible = False
        ScreenMask.Enabled = True
        ScreenMask.WaitData = True
        ScreenMask.Message = 'Filtros Avan'#231'ados'
        ScreenMask.Target = Owner
        TitleVisible = True
        Title = 'Filtros Avan'#231'ados'
        Caption = ''
        Collapsible = True
        Align = alTop
        Anchors = [akLeft, akTop, akRight]
        Color = clBtnFace
        TabOrder = 1
        object bt_pesquisa: TUniBitBtn
          Left = 670
          Top = 36
          Width = 80
          Height = 25
          Hint = 'Pesquisar'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Glyph.Data = {
            
            B6E4A3C9E4FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          Caption = 'Pesquisar'
          TabOrder = 1
          OnClick = bt_pesquisaClick
        end
        object rdb_pagas: TUniRadioButton
          Left = 601
          Top = 13
          Width = 62
          Height = 17
          Hint = ''
          Checked = False
          Caption = 'Pagas'
          TabOrder = 2
        end
        object rdb_a_pagar: TUniRadioButton
          Left = 601
          Top = 40
          Width = 62
          Height = 17
          Hint = ''
          Checked = True
          Caption = #192' Pagar'
          TabOrder = 3
        end
        object UniRadioButton1: TUniRadioButton
          Left = 601
          Top = 66
          Width = 62
          Height = 17
          Hint = ''
          Checked = False
          Caption = 'Ambos'
          TabOrder = 4
        end
        object data_inicial: TUniDateTimePicker
          Left = 402
          Top = 40
          Width = 89
          Height = 21
          Hint = ''
          DateTime = 41254.000000000000000000
          DateFormat = 'dd/MM/yyyy'
          TimeFormat = 'HH:mm:ss'
          Kind = tUniDate
          FirstDayOfWeek = dowLocaleDefault
          TabOrder = 5
          ParentColor = False
          Color = clWhite
          OnKeyPress = edt_pesq_descricaoKeyPress
        end
        object UniLabel9: TUniLabel
          Left = 402
          Top = 27
          Width = 53
          Height = 13
          Hint = ''
          Caption = 'Data Inicial'
          TabOrder = 6
        end
        object UniLabel10: TUniLabel
          Left = 498
          Top = 26
          Width = 48
          Height = 13
          Hint = ''
          Caption = 'Data Final'
          TabOrder = 7
        end
        object data_final: TUniDateTimePicker
          Left = 498
          Top = 40
          Width = 89
          Height = 21
          Hint = ''
          DateTime = 41254.000000000000000000
          DateFormat = 'dd/MM/yyyy'
          TimeFormat = 'HH:mm:ss'
          Kind = tUniDate
          FirstDayOfWeek = dowLocaleDefault
          TabOrder = 8
          ParentColor = False
          Color = clWhite
          OnKeyPress = edt_pesq_descricaoKeyPress
        end
        object edt_pesq_descricao: TUniEdit
          Left = 4
          Top = 57
          Width = 180
          Hint = ''
          PasswordChar = #0
          CharCase = ecNormal
          MaxLength = 0
          CharEOL = #0
          Text = ''
          TabOrder = 9
          Color = clWindow
          OnKeyPress = edt_pesq_descricaoKeyPress
        end
        object UniLabel6: TUniLabel
          Left = 4
          Top = 43
          Width = 46
          Height = 13
          Hint = ''
          Caption = 'Descri'#231#227'o'
          TabOrder = 10
        end
        object UniLabel8: TUniLabel
          Left = 207
          Top = 4
          Width = 55
          Height = 13
          Hint = ''
          Caption = 'Fornecedor'
          TabOrder = 11
        end
        object edt_pesq_fornecedor: TUniEdit
          Left = 207
          Top = 19
          Width = 180
          Hint = ''
          PasswordChar = #0
          CharCase = ecNormal
          MaxLength = 0
          CharEOL = #0
          Text = ''
          TabOrder = 12
          Color = clWindow
          OnKeyPress = edt_pesq_descricaoKeyPress
        end
        object edt_pesq_num_doc: TUniEdit
          Left = 4
          Top = 19
          Width = 180
          Hint = ''
          PasswordChar = #0
          CharCase = ecNormal
          MaxLength = 0
          CharEOL = #0
          Text = ''
          TabOrder = 13
          Color = clWindow
          OnKeyPress = edt_pesq_descricaoKeyPress
        end
        object UniLabel11: TUniLabel
          Left = 4
          Top = 4
          Width = 69
          Height = 13
          Hint = ''
          Caption = 'N'#186' Documento'
          TabOrder = 14
        end
        object UniLabel12: TUniLabel
          Left = 207
          Top = 43
          Width = 79
          Height = 13
          Hint = ''
          Caption = 'Tipo de Despesa'
          TabOrder = 15
        end
        object edt_pesq_tipo_despesa: TUniEdit
          Left = 207
          Top = 57
          Width = 180
          Hint = ''
          PasswordChar = #0
          CharCase = ecNormal
          MaxLength = 0
          CharEOL = #0
          Text = ''
          TabOrder = 16
          Color = clWindow
          OnKeyPress = edt_pesq_descricaoKeyPress
        end
      end
      object pn_meses: TUniPanel
        Left = 0
        Top = 0
        Width = 1164
        Height = 42
        Hint = ''
        Caption = ''
        Align = alTop
        Anchors = [akLeft, akTop, akRight]
        Color = clBtnFace
        TabOrder = 2
        object pn_ano: TUniPanel
          Left = 1
          Top = 1
          Width = 118
          Height = 40
          Hint = ''
          Caption = ''
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          Color = clBtnFace
          TabOrder = 1
          object bt_ano_seguinte: TUniBitBtn
            Left = 90
            Top = 1
            Width = 25
            Height = 38
            Cursor = crHandPoint
            Hint = 'Ano Seguinte'
            ShowHint = True
            ParentShowHint = False
            Caption = '<img width="14" height="28" src="../imagens/next.png" />'
            TabOrder = 1
            OnClick = bt_ano_seguinteClick
          end
          object bt_ano_anterior: TUniBitBtn
            Left = 1
            Top = 1
            Width = 25
            Height = 38
            Cursor = crHandPoint
            Hint = 'Ano Anterior'
            ShowHint = True
            ParentShowHint = False
            Caption = '<img width="14" height="28" src="../imagens/previous.png" />'
            TabOrder = 2
            OnClick = bt_ano_anteriorClick
          end
          object lb_ano: TUniLabel
            Left = 33
            Top = 9
            Width = 48
            Height = 23
            Hint = ''
            Caption = '2013'
            ParentFont = False
            Font.Color = clHotLight
            Font.Height = -19
            Font.Style = [fsBold]
            TabOrder = 3
          end
        end
        object bt_set: TUniBitBtn
          Left = 478
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Setembro'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/setembro.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 2
          OnClick = bt_janClick
        end
        object bt_mai: TUniBitBtn
          Left = 298
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Maio'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/maio.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 3
          OnClick = bt_janClick
        end
        object bt_abr: TUniBitBtn
          Left = 253
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Abril'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/abril.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 4
          OnClick = bt_janClick
        end
        object bt_mar: TUniBitBtn
          Left = 208
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Mar'#231'o'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/marco.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 5
          OnClick = bt_janClick
        end
        object bt_fev: TUniBitBtn
          Left = 163
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Fevereiro'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/fevereiro.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 6
          OnClick = bt_janClick
        end
        object bt_jan: TUniBitBtn
          Left = 118
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Janeiro'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/janeiro.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 7
          OnClick = bt_janClick
        end
        object bt_ago: TUniBitBtn
          Left = 433
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Agosto'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/agosto.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 8
          OnClick = bt_janClick
        end
        object bt_dez: TUniBitBtn
          Left = 613
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Dezembro'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/dezembro.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 9
          OnClick = bt_janClick
        end
        object bt_jul: TUniBitBtn
          Left = 388
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Julho'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/julho.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 10
          OnClick = bt_janClick
        end
        object bt_jun: TUniBitBtn
          Left = 343
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Junho'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/junho.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 11
          OnClick = bt_janClick
        end
        object bt_nov: TUniBitBtn
          Left = 568
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Novembro'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/novembro.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 12
          OnClick = bt_janClick
        end
        object bt_out: TUniBitBtn
          Left = 523
          Top = 1
          Width = 45
          Height = 40
          Hint = 'Outubro'
          ScreenMask.Enabled = True
          ScreenMask.WaitData = True
          ScreenMask.Message = 'Realizando a Pesquisa Solicitada, Aguarde...'
          ScreenMask.Target = dbg_pesquisa
          ShowHint = True
          ParentShowHint = False
          Caption = '<img width="32" height="32" src="../imagens/32/outubro.png" />'
          Align = alLeft
          Anchors = [akLeft, akTop, akBottom]
          TabOrder = 13
          OnClick = bt_janClick
        end
        object chk_filtros_avancados: TUniCheckBox
          Left = 677
          Top = 22
          Width = 116
          Height = 17
          Hint = ''
          Checked = False
          Caption = 'Filtros Avan'#231'ado'
          ParentFont = False
          Font.Color = clHotLight
          TabOrder = 14
          OnClick = chk_filtros_avancadosClick
        end
      end
    end
  end
  inherited UniStatusBar1: TUniStatusBar
    Top = 691
    Width = 1172
    ExplicitTop = 691
    ExplicitWidth = 1172
  end
  inherited DS: TDataSource
    DataSet = ADQuery
    Left = 1132
  end
  object DSPagamento: TDataSource
    DataSet = ADQueryPagamento
    Left = 338
    Top = 280
  end
  object Timer: TUniTimer
    OnTimer = TimerTimer
    Interval = 1000
    Enabled = False
    RunOnce = False
    ClientEvent.Strings = (
      'function(sender)'
      '{'
      '   '
      '}')
    Left = 952
    Top = 8
  end
  object DSAnexo: TDataSource
    DataSet = ADQueryAnexo
    Left = 1023
    Top = 166
  end
  object FileUpload: TUniFileUpload
    OnCompleted = FileUploadCompleted
    MaxAllowedSize = 10485760
    Title = 'Envio de Arquivos'
    Messages.Uploading = 'Enviando...'
    Messages.PleaseWait = 'Por Favor Aguarde...'
    Messages.Cancel = 'Cancela'
    Messages.Processing = 'Processando...'
    Messages.UploadError = 'Erro no Envio'
    Messages.Upload = 'Envia'
    Messages.NoFileError = 'Selecione o Arquivo'
    Left = 992
    Top = 8
  end
  object Popup: TUniPopupMenu
    Left = 1032
    Top = 8
    object ExportaParaExcel1: TUniMenuItem
      Caption = 'Exporta Para Excel'
      OnClick = ExportaParaExcel1Click
    end
  end
  object ADQuery: TADQuery
    CachedUpdates = True
    Connection = DM.ConexaoFiredac
    SQL.Strings = (
      'SELECT A.ID_MASTER,'
      '           A.ID_EMPRESA,'
      '           A.ID_CAP_AD,'
      '           B.ID_CAP_AP,'
      '           A.DESCRICAO_DOCUM,  '
      '           A.NUM_DOCUM,'
      '           A.SERIE_DOCUM,'
      '           A.TIPO_DOCUM,'
      '           A.TIPO_DOCUM_SITUACAO,'
      '           A.PERIODO,'
      '           A.NUM_PARCELAS,'
      '           B.PARCELA,'
      '           B.VAL_PARCELA,'
      '           IFNULL(B.VAL_JUROS,0)    VAL_JUROS,'
      '           IFNULL(B.VAL_MULTA,0)    VAL_MULTA,'
      '           IFNULL(B.VAL_DESCONTO,0) VAL_DESCONTO,'
      '           B.ID_TIPO_DESPESA,'
      '           C.DESCRICAO_DESPESA,'
      '           B.ID_F_C,'
      '           D.NOME RAZAO_SOCIAL,'
      '           B.DAT_VENCIMENTO,'
      '           B.DAT_PAGAMENTO,'
      '           B.TIP_PAGAMENTO,'
      '           (SELECT SUM(Y.VALOR) FROM CAP_AP_VALORES Y'
      '             WHERE A.ID_MASTER   = Y.ID_MASTER'
      '               AND A.ID_EMPRESA  = Y.ID_EMPRESA'
      '               AND B.ID_CAP_AP   = Y.ID_CAP_AP)VAL_PAGO '
      '      FROM CAP_AD                A,'
      '           CAP_AP                B,'
      '           TIPO_DESPESAS         C,'
      '           FORNECEDOR_CLIENTE    D        '
      '     WHERE A.ID_MASTER           = B.ID_MASTER'
      '       AND A.ID_EMPRESA          = B.ID_EMPRESA'
      '       AND A.ID_CAP_AD           = B.ID_CAP_AD'
      '       AND B.ID_MASTER           = C.ID_MASTER'
      '       AND B.ID_TIPO_DESPESA     = C.ID_TIPO_DESPESA'
      '       AND B.ID_MASTER           = D.ID_MASTER'
      '       AND B.ID_F_C              = D.ID_F_C'
      ''
      '       AND B.TIP_PAGAMENTO       IN ('#39'A'#39','#39'P'#39')'
      '       AND A.ID_MASTER            = :ID_MASTER'
      '       AND A.ID_EMPRESA           = :ID_EMPRESA')
    Left = 1096
    Top = 8
    ParamData = <
      item
        Name = 'ID_MASTER'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end
      item
        Name = 'ID_EMPRESA'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end>
    object ADQueryIMG_STATUS: TStringField
      DisplayLabel = '<img src="../imagens/16/indicador_bottom.png" />'
      DisplayWidth = 5
      FieldKind = fkCalculated
      FieldName = 'IMG_STATUS'
      OnGetText = ADQueryIMG_STATUSGetText
      Calculated = True
    end
    object ADQueryDAT_VENCIMENTO: TDateField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Vencimento'
      FieldName = 'DAT_VENCIMENTO'
      Origin = 'DAT_VENCIMENTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object ADQueryID_MASTER: TIntegerField
      FieldName = 'ID_MASTER'
      Origin = 'ID_MASTER'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
    end
    object ADQueryID_EMPRESA: TIntegerField
      FieldName = 'ID_EMPRESA'
      Origin = 'ID_EMPRESA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
    end
    object ADQueryID_CAP_AD: TADAutoIncField
      FieldName = 'ID_CAP_AD'
      Origin = 'ID_CAP_AD'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      IdentityInsert = True
    end
    object ADQueryID_CAP_AP: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'ID_CAP_AP'
      Origin = 'ID_CAP_AP'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
    end
    object ADQueryDESCRICAO_DOCUM: TWideStringField
      DisplayLabel = 'Descri'#231#227'o'
      DisplayWidth = 25
      FieldName = 'DESCRICAO_DOCUM'
      Origin = 'DESCRICAO_DOCUM'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 100
    end
    object ADQueryVAL_PARCELA: TBCDField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Valor a Pagar'
      DisplayWidth = 17
      FieldName = 'VAL_PARCELA'
      Origin = 'VAL_PARCELA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      currency = True
      Precision = 12
      Size = 2
    end
    object ADQueryVAL_PAGO: TFMTBCDField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Valor Pago'
      DisplayWidth = 17
      FieldName = 'VAL_PAGO'
      Origin = 'VAL_PAGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      currency = True
      Precision = 34
      Size = 2
    end
    object ADQueryDAT_PAGAMENTO: TDateField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Pagamento'
      FieldName = 'DAT_PAGAMENTO'
      Origin = 'DAT_PAGAMENTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object ADQueryIMG_TIPO: TStringField
      DisplayLabel = 'Tipo'
      DisplayWidth = 5
      FieldKind = fkCalculated
      FieldName = 'IMG_TIPO'
      OnGetText = ADQueryIMG_TIPOGetText
      Calculated = True
    end
    object ADQueryNUM_DOCUM: TWideStringField
      DisplayLabel = 'Documento'
      FieldName = 'NUM_DOCUM'
      Origin = 'NUM_DOCUM'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 9
    end
    object ADQuerySERIE_DOCUM: TWideStringField
      FieldName = 'SERIE_DOCUM'
      Origin = 'SERIE_DOCUM'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      Size = 3
    end
    object ADQueryTIPO_DOCUM: TWideStringField
      FieldName = 'TIPO_DOCUM'
      Origin = 'TIPO_DOCUM'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      Size = 45
    end
    object ADQueryTIPO_DOCUM_SITUACAO: TWideStringField
      FieldName = 'TIPO_DOCUM_SITUACAO'
      Origin = 'TIPO_DOCUM_SITUACAO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      Size = 45
    end
    object ADQueryPERIODO: TWideStringField
      FieldName = 'PERIODO'
      Origin = 'PERIODO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      Size = 45
    end
    object ADQueryPARCELA: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'PARCELA'
      Origin = 'PARCELA'
      ProviderFlags = []
      ReadOnly = True
    end
    object ADQueryNUM_PARCELAS: TIntegerField
      FieldName = 'NUM_PARCELAS'
      Origin = 'NUM_PARCELAS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
    end
    object ADQueryVAL_JUROS: TBCDField
      AutoGenerateValue = arDefault
      FieldName = 'VAL_JUROS'
      Origin = 'VAL_JUROS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      currency = True
      Precision = 12
      Size = 2
    end
    object ADQueryVAL_MULTA: TBCDField
      AutoGenerateValue = arDefault
      FieldName = 'VAL_MULTA'
      Origin = 'VAL_MULTA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      currency = True
      Precision = 12
      Size = 2
    end
    object ADQueryVAL_DESCONTO: TBCDField
      AutoGenerateValue = arDefault
      FieldName = 'VAL_DESCONTO'
      Origin = 'VAL_DESCONTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      Precision = 12
      Size = 2
    end
    object ADQueryID_TIPO_DESPESA: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'ID_TIPO_DESPESA'
      Origin = 'ID_TIPO_DESPESA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
    end
    object ADQueryDESCRICAO_DESPESA: TWideStringField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Tipo de Despesa'
      DisplayWidth = 25
      FieldName = 'DESCRICAO_DESPESA'
      Origin = 'DESCRICAO_DESPESA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 100
    end
    object ADQueryID_F_C: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'ID_F_C'
      Origin = 'ID_F_C'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
    end
    object ADQueryRAZAO_SOCIAL: TWideStringField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Fornecedor'
      DisplayWidth = 25
      FieldName = 'RAZAO_SOCIAL'
      Origin = 'NOME'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 100
    end
    object ADQueryTIP_PAGAMENTO: TWideStringField
      AutoGenerateValue = arDefault
      FieldName = 'TIP_PAGAMENTO'
      Origin = 'TIP_PAGAMENTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Visible = False
      FixedChar = True
      Size = 1
    end
  end
  object ADQueryPagamento: TADQuery
    Connection = DM.ConexaoFiredac
    SQL.Strings = (
      'SELECT A.ID_CAP_AP_VALORES,'
      '       A.DAT_PAGAMENTO,'
      '       A.VALOR,'
      '       B.TIPO,'
      '       B.NOME_CONTA,'
      '       C.NOME_BANCO'
      '  FROM CAP_AP_VALORES     A'
      '  LEFT OUTER JOIN CONTAS  B ON  A.ID_MASTER        = B.ID_MASTER'
      
        '                            AND A.ID_EMPRESA       = B.ID_EMPRES' +
        'A'
      '                            AND A.ID_CONTAS        = B.ID_CONTAS'
      
        '  LEFT OUTER JOIN BANCOS  C ON  B.ID_BANCOS        = C.ID_BANCOS' +
        ' '
      ' WHERE A.ID_MASTER        = :P00'
      '   AND A.ID_EMPRESA       = :P01'
      '   AND A.ID_CAP_AP        = :P02'
      ' ORDER BY A.DAT_PAGAMENTO')
    Left = 244
    Top = 279
    ParamData = <
      item
        Name = 'P00'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end
      item
        Name = 'P01'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end
      item
        Name = 'P02'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end>
    object ADQueryPagamentoID_CAP_AP_VALORES: TADAutoIncField
      FieldName = 'ID_CAP_AP_VALORES'
      Origin = 'ID_CAP_AP_VALORES'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object ADQueryPagamentoDAT_PAGAMENTO: TDateField
      FieldName = 'DAT_PAGAMENTO'
      Origin = 'DAT_PAGAMENTO'
      Required = True
    end
    object ADQueryPagamentoVALOR: TBCDField
      FieldName = 'VALOR'
      Origin = 'VALOR'
      Required = True
      currency = True
      Precision = 12
      Size = 2
    end
    object ADQueryPagamentoTIPO: TWideStringField
      AutoGenerateValue = arDefault
      FieldName = 'TIPO'
      Origin = 'TIPO'
      ProviderFlags = []
      ReadOnly = True
      Size = 45
    end
    object ADQueryPagamentoNOME_CONTA: TWideStringField
      AutoGenerateValue = arDefault
      FieldName = 'NOME_CONTA'
      Origin = 'NOME_CONTA'
      ProviderFlags = []
      ReadOnly = True
      Size = 100
    end
    object ADQueryPagamentoNOME_BANCO: TWideStringField
      AutoGenerateValue = arDefault
      FieldName = 'NOME_BANCO'
      Origin = 'NOME_BANCO'
      ProviderFlags = []
      ReadOnly = True
      Size = 100
    end
  end
  object ADQueryAnexo: TADQuery
    CachedUpdates = True
    Connection = DM.ConexaoFiredac
    SQL.Strings = (
      'SELECT * FROM CAP_AD_ANEXO A'
      ' WHERE A.ID_MASTER        = :P00'
      '   AND A.ID_EMPRESA       = :P01'
      '   AND A.ID_CAP_AD        = :P02')
    Left = 940
    Top = 165
    ParamData = <
      item
        Name = 'P00'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end
      item
        Name = 'P01'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end
      item
        Name = 'P02'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object ADQueryAnexoID_CAP_AD_ANEXO: TADAutoIncField
      AutoGenerateValue = arNone
      FieldName = 'ID_CAP_AD_ANEXO'
      Origin = 'ID_CAP_AD_ANEXO'
      ProviderFlags = [pfInUpdate]
      ServerAutoIncrement = False
      AutoIncrementSeed = 1
      AutoIncrementStep = 1
      IdentityInsert = True
    end
    object ADQueryAnexoNOME_ANEXO: TWideStringField
      FieldName = 'NOME_ANEXO'
      Origin = 'NOME_ANEXO'
      ProviderFlags = []
      Size = 100
    end
    object ADQueryAnexoANEXO: TBlobField
      FieldName = 'ANEXO'
      Origin = 'ANEXO'
      ProviderFlags = []
    end
  end
end
