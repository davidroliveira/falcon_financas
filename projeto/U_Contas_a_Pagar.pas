...
type
  ...
  private
    ...
    procedure atualiza_info_anexo(id_cap_ad :Integer);                          //Atualiza as Informações dos Anexos
    procedure status_rd_button;                                                 //Atualiza o Status do Rdb

    function documento_pago(id_ap :Integer): Boolean;                           //Verifica se AP Passado como Parâmetro está Paga.
  ...
  if (DS.DataSet.State in [dsEdit]) and (Not(nao_valida_edicao)) then
    begin
      if documento_pago(DS.DataSet.FieldByName('ID_CAP_AP').AsInteger) then
        begin
          ShowMessage('Para alterar este documento, é necessário excluir o pagamento existente.');
          DS.DataSet.Cancel;
          Exit;
        end;
...
begin
  ...
  if ID.alt_inf_pag then //Atualiza as Informações em Tela
    begin
      pesquisa(pesquisa_avancada,mes,ano);
      Timer.Enabled := False;
    end;
...
begin
  inherited;
  ShowMessage('Contas Normais Vencem apenas uma única vez. <br>  '+
              'Contas Recorrentes sempre irão existir, ex: conta de telefone, luz, mensalidade academia. <br> '+
